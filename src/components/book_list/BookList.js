...
export default class WordList extends Component {
    ...
    componentDidMount() {
        ...
        this.hanZi(window.AllBooksForPress, word)
    }


    /**
     *过滤汉字
     * @param currentBookData  当前书本的所有json数据
     * @param nowWords         当前系统抓取到的汉字
     */
    hanZi = (currentBookData, nowWords) => {
        let data = currentBookData;
        //出版社提供所有的汉字
        let arrWords = [];
        //当前抓取到的汉字
        nowWords = nowWords.replace(/[ ]/g, "").split("");
        //出版社汉字 我们这边没有抓取到的
        let noHaveWords = []
        for (let d of data) {
            for (let i of d.books) {
                // console.log(i)
                for (let j of i.book_data.lesson) {
                    // console.log(j)
                    for (let h of j.words) {
                        // console.log(h)
                        arrWords.push(h.word)
                    }
                }
            }
        }
        // console.log("nowWords......", nowWords.indexOf("央"))
        for (let i of arrWords) {
            let a = nowWords.indexOf(i)
            if (a < 0) {
                noHaveWords.push(i)
            }
        }
        let obj = {}
        obj.hanZi = noHaveWords.join();
        obj.arrHanZi = noHaveWords;
        // console.log(obj)
        return obj;
    ...
    render() {
        let {bookData, pressname} = this.state;
                            ...
                            return <Link to={`/press/${presspinyin}/book/${index}`} key={"bookItem" + index}>
                                <div className="bookItem">
                                    <div className="bookImg" style={{width: '90px', marginRight: "15px"}}>
                                        <img style={{width: "100%", height: "100%"}} src={item.book_data.courseBanner}
                                             alt="图书封面"/>
                                    </div>
